...
st.sidebar.markdown("## 📝 Register as a New Student")
...
if st.session_state.get("register"):
    ...
    if st.button("Submit Registration"):
        if reg_email and reg_name and reg_pass:
            st.success(f"Registered {reg_name} — please contact admin to activate your access.")
        else:
...
if st.session_state.get("reset"):
    ...
    if st.button("Update Password"):
        st.success(f"If {reset_email} is valid, your password will be updated by admin.")
